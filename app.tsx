    ...
    <div className="max-w-6xl mx-auto px-4 py-12 min-h-screen">
      <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-[#800000]">
        ...
        <div className="flex flex-col md:flex-row h-[70vh]">
          <div className="w-full md:w-64 bg-gray-100 border-r p-4 space-y-2 overflow-y-auto hide-scrollbar">
            {[{ id: 'homepage', label: 'होम और दर्शन', icon: Home }, { id: 'gallery', label: 'गैलरी प्रबंधन', icon: ImageIcon }, { id: 'aarti', label: 'आरती समय', icon: Calendar }].map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full text-left px-4 py-3 rounded-xl flex items-center ${activeTab === tab.id ? 'bg-[#FF9933] text-white shadow-md' : 'hover:bg-gray-200'}`}><tab.icon size={18} className="mr-3" /> {tab.label}</button>
            ))}
          </div>
          <div className="flex-1 p-8 overflow-y-auto hide-scrollbar">
            {activeTab === 'homepage' && (
              <div className="space-y-6">
                ...
                <textarea className="w-full p-3 border rounded-lg h-24 focus:ring-2 focus:ring-orange-200 outline-none" value={data.homepage.dailyThought} onChange={e => updateField('homepage', 'dailyThought', e.target.value)} />
              </div>
            )}
            {activeTab === 'gallery' && (
              <div className="space-y-6">
            ...
            {activeTab === 'aarti' && (
              <div className="space-y-4">
                {data.aartiTimings.map((t, idx) => (
                  <div key={t.id} className="flex gap-4 items-center bg-gray-50 p-4 rounded-xl border">
                    <input className="flex-1 p-2 border rounded outline-none focus:ring-2 focus:ring-orange-200" value={t.name} onChange={e => {
                      const newT = [...data.aartiTimings]; newT[idx].name = e.target.value; onUpdateData({ ...data, aartiTimings: newT });
                    }} />
                    <input className="w-32 p-2 border rounded outline-none focus:ring-2 focus:ring-orange-200" value={t.time} onChange={e => {
      ...
      if (!isAdmin) {
        return (
          <div className="flex items-center justify-center min-h-[60vh] px-4">
            <div className="bg-white p-8 rounded-3xl shadow-2xl border-t-8 border-[#800000] w-full max-w-md">
              ...
              <form onSubmit={e => {
                ...
                <input type="text" className="w-full px-4 py-2 border rounded-lg outline-none focus:ring-2 focus:ring-orange-200" value={adminUser} onChange={e => setAdminUser(e.target.value)} placeholder="यूजरनाम" />
                <input type="password" className="w-full px-4 py-2 border rounded-lg outline-none focus:ring-2 focus:ring-orange-200" value={adminPass} onChange={e => setAdminPass(e.target.value)} placeholder="पासवर्ड" />
                {loginError && <p className="text-red-500 text-sm">{loginError}</p>}
                <button type="submit" className="w-full bg-[#800000] text-white py-3 rounded-lg font-bold hover:bg-[#600000] transition-colors">लॉगिन</button>
              </form>
            </div>
          </div>
        );
